...
- On pull requests every app CI starts and its `changes` job decides whether the rest runs (a conditionally skipped job counts as passed), so the per-app checks may be required on `main`.
